...
# Mappa di adiacenza  [sw1][sw2] -> port da sw1 a sw2

adjacency = defaultdict(lambda:defaultdict(lambda:None))

 

# Switches noti.  [dpid] -> Switch
...
# Temp consentito per impostare un percorso
...
def _check_path (p):
  ...
  Si assicura che il path sia una stringa di nodi con le porte collegate

  Ritorna True se il path è valido
...
def _get_path (src, dst, first_port, final_port):
  ...
  Ottiene un path -- una lista di (node,in_port,out_port)

  """

  # Inizia con un percorso grezzo
...
class PathInstalled (Event):
  ...
  Attivato quando viene installato un percorso
...
class Switch (EventMixin):
  ...
  def install_path (self, dst_sw, last_port, match, event):
    ...
    Tenta di installare un percorso tra questa opzione e una destinazione
    ...
    if p is None:
      ...
      if (match.dl_type == pkt.ethernet.IP_TYPE and
        ...
        # Inviamo una destinazione irraggiungibile
